    ...
    //阻断
      ...
      outBody = JSON.stringify({
        ...
        msg:
          ...
          ' was block-listed by the operator of this proxy.',
      })
      outCt = 'application/json'
      outStatus = 403
    } else {
      ...
      if (blockType(outCt)) {
        outBody = JSON.stringify({
          ...
          msg:
            'The keyword "' +
            config.typeList.join(' , ') +
            '" was whitelisted by the operator of this proxy, but got "' +
            outCt +
            '".',
        })
        outCt = 'application/json'
        ...
        for (let h of fr.headers.entries()) {
          if (!overrideHeaders.has(h[0])) outHeaders.set(h[0], h[1])
        }
      }
    }
  } catch (err) {
...
async function werssHandler(request, url) {
  ...
  try {
    ...
    if (!fr.ok)
      throw new Error('upstream status: ' + fr.status + ' ' + fr.statusText)
...
async function buildFullTextFeed(xml, format, feedUrl) {
  ...
  const max = Math.min(50, Math.max(0, parseInt(config.werssMaxItems, 10) || 0))
  const items = parsed.items.slice(0, max)

  const enriched = await mapWithConcurrency(
    items,
    config.werssConcurrency,
    async (item) => {
  ...
  enriched.forEach((it) => {
    const body =
      (it.fullText && it.fullText.content) || it.content || it.description || ''
    out += '<item>\n'
    if (it.title) out += '<title>' + cdata(it.title) + '</title>\n'
    if (it.link) out += '<link>' + esc(it.link) + '</link>\n'
    if (it.guid)
      out += '<guid isPermaLink="false">' + esc(it.guid) + '</guid>\n'
